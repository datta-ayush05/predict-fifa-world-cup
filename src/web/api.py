...
DATA_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "..", "data", "predictions.json"
)
...
def load_data():
    if not os.path.exists(DATA_PATH):
        raise HTTPException(
            status_code=404,
            detail="Prediction data not found. Please run the model simulation first.",
        )
...
def get_team_stats(team_name: str):
    ...
    return {"stats": stats, "group_matches": matches, "n_sims": data.get("n_sims", 0)}
